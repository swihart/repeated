...
#  SYNOPSIS
#
#     chidden(response=NULL, totals=NULL, times=NULL, distribution="Bernoulli",
...
##' @param pgamma A square \code{mxm} matrix of initial estimates of the
...
tmp <- c("Bernoulli","Poisson","multinomial","continuation ratio",
	 ...
	 "skew Laplace","Student t","discretized Poisson process")
...
if(states>1)for(i in 1:states){
	if(pgamma[i,i]!=0&&round(sum(pgamma[i,]),2)!=0)
		stop(paste("row",i,"of pgamma does not sum to 0"))
	for(j in 1:states)if(i!=j&&pgamma[i,j]!=0)
		ipos <- c(ipos,states*(j-1)+i)
	pg <- c(pg,pgamma[i,(1:states)!=i&pgamma[i,]!=0])}
if(any(pg<=0))stop("off-diagonal elements of pgamma must be non-negative")
#
...
if(respenv){
	if(dim(response$response$y)[2]>1){
		if(mdl==3)mdl <- 4
		else stop("chidden only handles univariate responses")}
	if(!is.null(response$NAs)&&any(response$NAs))
		stop("chidden does not handle data with NAs")
...
if(!is.null(response$response$nest)&&!is.null(par))
	stop("chidden does not handle two levels of nesting with an AR")
if(!is.null(response$response$censor))stop("chidden does not handle censoring")
...
p <- c(log(pg),pmu,pcmu,ptvmu,pshape,pfamily,pintercept,par)
np <- length(p)

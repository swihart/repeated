...
#  SYNOPSIS
#
#     gnlmix_dotcall(y=NULL, distribution="normal", mixture="normal",
...
##' 
##' \code{gnlmix_dotcall} fits user-specified nonlinear regression equations to one or
...
##' gnlmix_dotcall(reps, mu=~a+b*dose+rand, random="rand", pmu=c(8.7,0.25),
...
##' 	psh=1, psd=0.3)
##' gnlmix_dotcall(reps, distribution="gamma", mu=~exp(a+b*dose+rand), random="rand",
##' 	pmu=c(2,0.04), pshape=1, pmix=-2)
##' 
##' # gamma model with log link and random gamma mixtures
##' gnlmix_dotcall(reps, distribution="gamma", mixture="gamma",
...
gnlmix_dotcall <- function(y=NULL, distribution="normal", mixture="normal",
                           random=NULL, nest=NULL, mu=NULL, shape=NULL, linear=NULL,
                           pmu=NULL, pshape=NULL, pmix=NULL, delta=1, common=FALSE,
                           envir=parent.frame(), print.level=0, typsize=abs(p),
                           ndigit=10, gradtol=0.00001, stepmax=10*sqrt(p%*%p), steptol=0.00001,
  ...
  if(respenv){
    if(dim(y$response$y)[2]>1)
      stop("gnlmix_dotcall only handles univariate responses")
    if(!is.null(y$NAs)&&any(y$NAs))
      stop("gnlmix_dotcall does not handle data with NAs")}
  ...
  if(respenv){
    ...
    if(dim(y$y)[2]>1)stop("gnlmix_dotcall only handles univariate responses")
